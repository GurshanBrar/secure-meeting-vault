...
def create(request, member_count=None):
    ...
    if member_count == None:
        ...
        raise Http404
    if request.method == 'POST':
        form = forms.CreateForm(request.POST,request.FILES, member_count=member_count)
        if form.is_valid():
            ...
            group = Group.objects.create(group_id = group_id, meeting_url = meeting_url)
            # take care of getting member count
            for i in range(member_count):
                name = form.cleaned_data[f'name_{i}']

                image = form.cleaned_data[f'image_{i}']
                person = (face_recognition.face_encodings(face_recognition.load_image_file(image))[0])
        ...
        if form.is_valid():
            ...
            for i in matching_names:
                ...
                if face_recognition.compare_faces([test], user):
                    face_distances = face_recognition.face_distance([test], user)
